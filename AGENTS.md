...
- The idle advisor page renders no result placeholder region or result-placeholder text. Omit the `MODEL ADVISOR` kicker; render the Chinese `按需求选模型` title in the advisor purple, then only the requirement field, optional deployment-region field, their labels/placeholders/validation errors, and the submit action. Place `获取推荐` to the right of the deployment-region field.
