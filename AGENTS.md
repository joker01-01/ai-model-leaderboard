...
- DeepSeek may return result-derived search queries outside the finite approved query set. Fully validate each search action before deciding its disposition; if any query is unknown, ignore the entire action atomically. An ignored search invalidates any first-response message, so continue at most once using only independent completed search actions whose queries are all recognized. Never replay ignored, failed, navigation, or continuation-marker actions from that response, and never log raw query text.
