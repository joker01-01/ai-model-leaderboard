...
- First detail entry and changes to ability metric, creator filter, efficiency metric or sort direction replay a shared 600ms timeline and respect reduced motion. Animate entry-viewport bars with Web Animations transforms and one RAF for visible number text; never rerender the full React ranking or change layout widths each frame. Offscreen rows and unsupported-animation browsers show final values. Unrelated rerenders and clicking an already selected control without changing state do not replay it.
