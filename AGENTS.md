...
- The idle advisor page renders no result placeholder region or result-placeholder text. Omit the `MODEL ADVISOR` kicker; render the Chinese `按需求选模型` title in the advisor purple, then only the requirement field, optional deployment-region native select, their labels/placeholders/validation errors, and the submit action. Place `获取推荐` to the right of the deployment-region select.
- Inset the Advisor back control by the same 32px as the shell's left edge instead of pinning it to the viewport edge; preserve that alignment in the scaled phone layout.
- The public deployment-region select defaults to `不指定` and sends `null`. Its fixed Chinese-label-to-wire-value mappings are `中国大陆` -> `Mainland China`, `中国香港` -> `Hong Kong`, `新加坡` -> `Singapore`, `日本` -> `Japan`, `韩国` -> `South Korea`, `美国` -> `United States`, `加拿大` -> `Canada`, `欧盟` -> `European Union`, `英国` -> `United Kingdom`, and `澳大利亚` -> `Australia`. These values express unverified user preference only; they never imply provider availability.
- The public browser form never exposes budget controls and always sends `budget: null`. Keep the backend/API request contract compatible with an optional validated budget object for non-browser callers; this compatibility must not add budget fields back to the public form.
- Keep the backend/API `deployment_region` request contract compatible with any clean string or `null` for non-browser callers. The browser's fixed select values must not narrow that API contract, and deployment region remains unverified and excluded from filtering and ordering.
- DeepSeek may extract a strict intent/constraint contract but does not choose or rank arbitrary models.
- Its output is limited to ordered ability enums, one promoted objective, and reviewed hard-requirement enums; deployment region and any optional budget/token values come only from the validated request. Reject model-provided URLs, candidate/provider IDs, unknown fields, and unsupported enums.
