...
- Advisor production compatibility fixes merged through PRs #27-#35: the timeout is 60 seconds, validated provider reformulations and bounded continuation markers are replayed, unregistered navigation is ignored without becoming evidence, and the second response is constrained to one strict JSON object.
- PR #36 added bounded raw/parsed citation-annotation diagnostics without widening any evidence gate. PR #37 added the explicit Uvicorn logging configuration required for those `app.*` INFO events to reach production logs; runtime code baseline `4b9e5f3` is published.
- Zeabur deployment `deployment-6a9cc58caad15df0678d3f30` ran the PR #37 baseline with one healthy instance. A public advisor request on 2026-09-06 returned a schema-valid deterministic `aa_only` recommendation plus two alternatives in 29.73 seconds.
...
- These are recorded checks of their respective revisions. The strict citation gates and production logging path are covered; no live accepted official citation exists with the current DeepSeek Responses behavior.

## Next

- Re-run one bounded production probe only after the provider contract changes; compare the safe raw/valid/accepted counters without recording response content.
- Merge PR #18 only after explicit acceptance of the AA v4.2 scale change, the Agentic 197-to-100 coverage reduction, and the resulting advisor-candidate impact. After that acceptance, rerun the signed refresh from then-current `main` and review its regenerated head before manual merge; the existing head predates PRs #36-#37. Missing Agentic values remain missing, never zero.
