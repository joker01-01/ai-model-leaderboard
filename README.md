...
编辑推荐榜保留六个维度：综合智能、编程、工具使用、推理·数学、性价比和开源。前四项来自同版本公开数据，性价比按价格档位规则计算，开源按“已公开权重=100、未公开或待核实=0”的二值规则计算；API 可用不等于模型权重开放。你可以直接使用预设，也可以调整权重，比较不同偏好下的排序变化。默认口径中开源权重为 2%，避免许可证状态压过实际能力表现。没有同版本 AA 智能指数的模型不会使用手工分数替代，会进入“待补编辑基础分”区。
...
- `src/data/models.ts`：模型资料、价格、许可证与来源
- `src/lib/editorial.ts`：编辑榜六项基础分的来源与固定规则
...
公开评测榜是可追溯的公开快照，不等同于厂商官方结论，也不能覆盖所有 benchmark。编辑推荐榜的排序由用户权重决定，但基础分来自公开数据与固定规则；数据不完整时会明确显示待补。模型版本、价格、上下文和许可证变化很快，实际使用前请回到模型厂商和原始评测页面复核。
