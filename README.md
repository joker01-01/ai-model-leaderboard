...
编辑推荐榜保留六个维度：综合智能、编程、工具使用、推理·数学、性价比和开源。前四项来自同版本公开数据，性价比按价格档位规则计算，开源按“开放权重=100、未开放权重=0”的二值规则计算；你可以直接使用预设，也可以调整权重，比较不同偏好下的排序变化。默认口径中开源权重为 2%，避免许可证状态压过实际能力表现。没有同版本 AA 智能指数的模型不会使用手工分数替代，会进入“待补编辑基础分”区。
