...
import type { Entry, RankingMode, SortKey } from "../lib/entries";
import { entryValue, observationsFor } from "../lib/entries";
import { DIM_KEYS, DIM_LABELS, OBJECTIVE_DIM_KEYS, OBJECTIVE_DIM_LABELS, type ObjectiveDimKey } from "../lib/score";
import Radar from "./Radar";

/* ---------- 榜单 ---------- */
export function Board({ mode, entries, sortKey, expanded, onToggle }: {
  ...
  return (
    <section className="board" aria-label={`${mode === "objective" ? "公开评测榜" : "编辑推荐榜"}排名`}>
      ...
      <ol className="rows">{rankedEntries.map((entry) => {
        // 同分并列同名次：与排序键无关，两个榜单口径一致
        const rank = rankedEntries.findIndex((candidate) => entryValue(candidate, mode, sortKey) === entryValue(entry, mode, sortKey)) + 1;
