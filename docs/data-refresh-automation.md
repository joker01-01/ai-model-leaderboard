...
6. Automatic merge requires a previously reviewed full-snapshot baseline, stable public schema/wire fingerprint and existing identity metadata, semantically equal TypeScript/JSON snapshots, consistent pagination/report proof, no greater-than-20-percent total-row or per-metric coverage loss, stable curated exact-match membership and source identities, no ambiguity or skipped source, unchanged curated/static evidence, and successful required checks. After the baseline, ordinary public model additions/removals and metric value/date/order changes may be routine. The positive finite AA Intelligence Index version follows upstream and may change without manual rebaselining; same-fetch pagination and generated-artifact consistency remain mandatory. Structural changes and threshold violations require human review.
...
- Final full-snapshot refresh run [`34005442709`](https://github.com/joker01-01/ai-model-leaderboard/actions/runs/34005442709) regenerated [PR #18](https://github.com/joker01-01/ai-model-leaderboard/pull/18) from `main` at `a0f5e7b` as App-signed head `ee615ac`, with 643 unique public rows and complete four-page pagination. Its [`verify` run](https://github.com/joker01-01/ai-model-leaderboard/actions/runs/34005499640) passed. Trusted policy run [`34005549017`](https://github.com/joker01-01/ai-model-leaderboard/actions/runs/34005549017) correctly retained the PR for human review because Intelligence changed from version 4.1 to 4.2, Agentic finite coverage fell from 197 to 100, and legacy compatibility membership changed. This is an expected fail-closed result, not an automation failure.

Together, PR #7 and PR #9 prove fail-closed anomaly handling and unattended routine publication for the curated snapshot pipeline. PR #11 proves the human-reviewed legacy 20-row membership/version baseline. The first 643-row complete public baseline was human reviewed on 2026-09-04, and PR #18 proves that the complete snapshot path also retains a methodology/coverage anomaly after all technical checks pass. A later qualifying routine full-snapshot change is still needed to prove unattended auto-merge for that path. If a future run has no generated diff, a successful no-PR result is expected but does not add merge-path evidence.
