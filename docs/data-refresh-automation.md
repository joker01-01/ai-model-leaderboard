...
The repository-scoped GitHub App, Actions credentials, protected `main` rules, anomaly-retention path, and routine automatic-merge path are operational and were live-verified on 2026-09-04.
...
6. Automatic merge requires stable exact-match membership, stable source/model identities, no ambiguity or skipped source, unchanged curated/static evidence, successful required checks, exactly one App-authored GitHub-signed commit with the expected `web-flow` committer, fixed message, valid verification and parent SHA, and unchanged PR head/base/current-`main` SHAs.
...
Together, PR #7 and PR #9 prove both fail-closed anomaly handling and unattended routine publication. If a future run has no generated diff, a successful no-PR result is expected but does not add merge-path evidence.
