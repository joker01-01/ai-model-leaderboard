...
DeepSeek's Responses `web_search` tool is accessed only through a server-side adapter. Search queries and URL acceptance come from a reviewed `creatorId` source registry, never from a user-provided URL or model-invented domain. Returned search actions may contain an exact server-generated query, a bounded provider continuation marker, or a reformulation derived from one frozen candidate's complete name or its base name after removing only a recognized configuration suffix, paired with the matching creator/AA registry scope. A quoted reformulation may additionally end with the one literal lowercase token `model`, an observed provider-neutral query hint; no other appended text is accepted. Other partial names, appended text, cross-creator scopes, and unregistered domains fail closed. As a narrow compatibility exception for an observed production wire response, a provider-reported failed `open_page` action is tolerated only after the target URL passes the same reviewed registry binding; that action metadata does not satisfy the completed-search requirement or become evidence itself. Other failed or incomplete web actions remain fatal. A search summary is not evidence until every cited URL passes the registry binding.
